...
import {
  fetchGroups,
  searchGroups,
} from "../../../redux/nodeSlice/getContactGroupSlice";
...
const ContactGroups = () => {
  ...
  const groupsState: any = useSelector(
    (state: RootState) => state && state.groups
  );
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIds = paginatedData.map((row: any) => row.id);
